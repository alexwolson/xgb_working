...
def generate_csv_files(data_directory: str, sen_geometrical: bool = False,clogging_factors: bool = False) -> None:
    ...
    excel_files = [file for folder in tabular_root.glob('**/*') if folder.is_dir() for file in folder.glob('**/*.xlsx')]
...
def main():
    ...
    parser.add_argument('--evaluate-only', action='store_true', help='Only run evaluate_model and build the training config JSON. To be used after multithreaded training.')
